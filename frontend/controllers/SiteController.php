...
class SiteController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => ['logout', 'signup', 'comment-moderation', 'success-comment'],
                'rules' => [
                    [
                        'actions' => ['signup'],
                        'allow' => true,
                    ...
                    [
                        'actions' => ['logout', 'comment-moderation', 'success-comment'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    // Komentari tekshiruvi 
    public function actionCommentModeration()
    {
        $model = Commit::find()
            ->where(['status' => 0])
            ->all();
        ...
        return $this->render('comment-moderation', [
            'comment' => $model,
        ]);
    }
    ...
    public function actionRejectionComment($id)
    {
        ...
        $model->status = 2;
        $model->save();
        Yii::$app->session->setFlash('success', 'Komentari rad etildi');
        return $this->redirect(['/site/comment-moderation']);
    }
